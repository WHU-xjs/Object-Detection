...
class DetectionNetwork(object):
    ...
    def rpn_cls_net(self, inputs, scope_list, reuse_flag, level):
        ...
        att_rpn_conv2d_3x3 = rpn_conv2d_3x3

        att_rpn_conv2d_3x3 = slim.conv2d(inputs=rpn_conv2d_3x3,
                                         num_outputs=256,
                                         kernel_size=[3, 3],
                                         stride=1,
                                         activation_fn=tf.nn.relu,
                                         weights_initializer=cfgs.SUBNETS_WEIGHTS_INITIALIZER,
                                         biases_initializer=cfgs.SUBNETS_BIAS_INITIALIZER,
                                         scope='{}_{}'.format(scope_list[0], i+4),
                                         reuse=reuse_flag)

        att_rpn_conv2d_3x3 = tf.sigmoid(att_rpn_conv2d_3x3)
 
        rpn_conv2d_3x3 = tf.multiply(att_rpn_conv2d_3x3,rpn_conv2d_3x3)

        add_heatmap(rpn_conv2d_3x3, name='h_cls_head_%s' % level)
    ...
    def rpn_reg_net(self, inputs, scope_list, reuse_flag, level):
        ...
        att_rpn_delta_boxes=rpn_conv2d_3x3

        for i in range(4):
            att_rpn_delta_boxes = slim.conv2d(inputs=att_rpn_delta_boxes,
                                          num_outputs=256,
                                          kernel_size=[3, 3],
                                          weights_initializer=cfgs.SUBNETS_WEIGHTS_INITIALIZER,
                                          biases_initializer=cfgs.SUBNETS_BIAS_INITIALIZER,
                                          stride=1,
                                          activation_fn=tf.nn.relu,
                                          scope='{}_{}'.format(scope_list[1], i+4),
                                          reuse=reuse_flag)

        att_rpn_delta_boxes = tf.sigmoid(att_rpn_delta_boxes)

        rpn_conv2d_3x3=tf.multiply(rpn_conv2d_3x3,att_rpn_delta_boxes)

        add_heatmap(rpn_conv2d_3x3, name='h_reg_head_%s' % level)
